...
@configclass
class Go2lbl2RoughSceneCfg(InteractiveSceneCfg):
...
class Go2lbl2RoughRewards:
    ...
    gait_ensure = RewTerm(
        func=mdp_nhb.QuadrupedGaitEnsureReward,
        weight=1.0,
        params={
            "lf_sensor_cfg": SceneEntityCfg("contact_forces", body_names="FL_foot"),
            "rf_sensor_cfg": SceneEntityCfg("contact_forces", body_names="FR_foot"),
...
@configclass
class Go2lbl2RoughEventCfg:
    """Configuration for events."""
    # startup
    rigid_body_material = EventTerm(
    ...
    rigid_body_mass_base = EventTerm(
    ...
    # rigid_body_mass_others = EventTerm(
    #     func=mdp.randomize_rigid_body_mass,
    #     mode="startup",
    #     params={
    ...
    joint_friction = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=".*"),
            "friction_distribution_params": (0.01, 0.25),
...
class Go2lbl2RoughCommandsCfg:
    ...
    gait_command = mdp_nhb.QuadrupedGaitCommandCfg(
        ...
        ranges=mdp_nhb.QuadrupedGaitCommandCfg.Ranges(
            ...
            gait_frequency=(1.25, 1.25)  # 步态频率1.25Hz
        ),
    )     

@configclass
class Go2lbl2RoughTerminationsCfg:
    """复位触发条件"""
...
@configclass
class Go2lbl2RoughCurriculumCfg:
    """课程"""
...
@configclass
class Go2lbl2RoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    # 开启图形渲染时才使用绘图窗口
    if not isaaclab_nhb.HEADLESS_FLAG:
        ui_window_class_type:Go2DebugWindow = Go2DebugWindow
    scene: Go2lbl2RoughSceneCfg = Go2lbl2RoughSceneCfg(num_envs=4096, env_spacing=2.5)
    # Basic settings
    observations: Go2lbl2RoughObsCfg = Go2lbl2RoughObsCfg()
    actions: Go2lbl2RoughActionsCfg = Go2lbl2RoughActionsCfg()
    commands: Go2lbl2RoughCommandsCfg = Go2lbl2RoughCommandsCfg()
    # MDP settings
    rewards: Go2lbl2RoughRewards = Go2lbl2RoughRewards()
    terminations: Go2lbl2RoughTerminationsCfg = Go2lbl2RoughTerminationsCfg()
    events: Go2lbl2RoughEventCfg = Go2lbl2RoughEventCfg()
    curriculum: Go2lbl2RoughCurriculumCfg = Go2lbl2RoughCurriculumCfg()
...
@configclass
class Go2lbl2FlatEnvCfg(Go2lbl2RoughEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
@configclass
class Go2lbl3RoughRewards(Go2lbl2RoughRewards):
    """Go2lbl3奖励函数配置 - 添加四足mimic奖励"""
    ...
    mimic_rew = RewTerm(
        ...
        weight=5.0,
        params={
            "command_name": "mimic_command",
            "asset_cfg": SceneEntityCfg("robot", joint_names=GO2_JOINT_NAMES, preserve_order=True),
            "std": 0.7
        },
    )
...
@configclass
class Go2lbl3RoughCommandsCfg(Go2lbl2RoughCommandsCfg):
    """Go2lbl3命令配置 - 添加四足mimic命令"""
    
    # 模仿命令 - 使用转换后的Go2 npz数据
    mimic_command = mdp_nhb.QuadrupedMimicCommandCfg(
        resampling_time_range=(1500.0, 1500.0),
        data_path="/home/jyz/project/isaaclab_nhb_ws/isaaclab_nhb/isaaclab_nhb/dataset/Go2-lbl/Go2_data.npz",
    )


@configclass
class Go2lbl3RoughCurriculumCfg(Go2lbl2RoughCurriculumCfg):
    """Go2lbl3课程配置 - 添加mimic命令标准差课程"""
    
    # Mimic命令标准差课程学习
    mimic_cmd_std_levels_exp = CurrTerm(
        func=mdp_nhb.lin_vel_cmd_std_levels,
        params={
            "reward_term_name": "mimic_rew",
            "step": 0.01,
            "min_std": 0.1
        }
    )
...
@configclass
class Go2lbl3RoughEnvCfg(Go2lbl2RoughEnvCfg):
    """Go2lbl3环境配置 - 有步态 + 有mimic"""
    
    commands: Go2lbl3RoughCommandsCfg = Go2lbl3RoughCommandsCfg()
    rewards: Go2lbl3RoughRewards = Go2lbl3RoughRewards()
    # curriculum: Go2lbl3RoughCurriculumCfg = Go2lbl3RoughCurriculumCfg()
...
@configclass
class Go2lbl3FlatEnvCfg(Go2lbl3RoughEnvCfg):
    """Go2lbl3平地环境配置"""
...
@configclass
class Go2lbl1RoughObsCfg(Go2lbl2RoughObsCfg):
    """Go2lbl1观测配置 - 删除gait_commands"""
    
    @configclass
    class PolicyCfg(Go2lbl2RoughObsCfg.PolicyCfg):
        """Policy观测 - 无gait命令"""
        gait_commands = None
    
    @configclass
    class CriticCfg(Go2lbl2RoughObsCfg.CriticCfg):
        """Critic观测 - 无gait命令"""
        gait_commands = None
...
@configclass
class Go2lbl1RoughRewards(Go2lbl2RoughRewards):
    """Go2lbl1奖励配置 - 删除gait相关奖励"""
...
@configclass
class Go2lbl1RoughCommandsCfg(Go2lbl2RoughCommandsCfg):
    """Go2lbl1命令配置 - 删除gait_command"""
...
@configclass
class Go2lbl1RoughEnvCfg(Go2lbl2RoughEnvCfg):
    """Go2lbl1环境配置 - 无步态无mimic，仅速度跟踪"""
    
    observations: Go2lbl1RoughObsCfg = Go2lbl1RoughObsCfg()
    commands: Go2lbl1RoughCommandsCfg = Go2lbl1RoughCommandsCfg()
    rewards: Go2lbl1RoughRewards = Go2lbl1RoughRewards()
...
@configclass
class Go2lbl1FlatEnvCfg(Go2lbl1RoughEnvCfg):
    """Go2lbl1平地环境配置"""
