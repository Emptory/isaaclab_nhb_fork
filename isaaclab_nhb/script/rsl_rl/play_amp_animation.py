...
def play_single_file(AMPwrapped_env, file_path, args_cli, DataCfg, simulation_app):
    ...
    while frame_cnt < AMPwrapped_env.motion_len and simulation_app.is_running():
        ...
        if DataCfg.save_motion_expert_amp_data and args_cli.save_path and frame is not None:
            ...
            # 提取root速度（假设在frame中的索引为35:38，即lin_vel）
            # 根据代码中visualize_motion的逻辑：visual_motion_frame[35:38]是lin_vel
            visual_motion_frame = AMPwrapped_env.amp_loader_display.get_full_frame_at_time(0, t)
            root_vel_x = visual_motion_frame[35].item()  # x方向速度
            root_vel_y = visual_motion_frame[36].item()  # y方向速度
            root_vel_x_list.append(root_vel_x)
            root_vel_y_list.append(root_vel_y)
    ...
    if DataCfg.save_motion_expert_amp_data and args_cli.save_path and len(all_frames) > 0:
        # 计算平均速度：sqrt(vx^2 + vy^2)
        avg_speed = 0.0
        if len(root_vel_x_list) > 0:
            avg_vx = np.mean(root_vel_x_list)
            avg_vy = np.mean(root_vel_y_list)
            avg_speed = np.sqrt(avg_vx**2 + avg_vy**2)

        # 获取文件所在文件夹的名字
        # file_path是完整路径，例如：/path/to/visualization_path/subfolder/file.txt
        # 我们需要获取相对于visualization_path的文件夹名
        relative_path = os.path.relpath(file_path, DataCfg.visualization_path)
        folder_name = os.path.dirname(relative_path)

        # 如果文件直接在visualization_path下（没有子文件夹），使用文件名（去掉扩展名）
        ...
        # 生成新的文件名：文件夹名_平均速度.txt
        new_filename = f"{folder_name}_{avg_speed:.3f}ms.txt"
        ...
        current_save_path = os.path.join(save_dir, new_filename)

        print(f"💾 正在保存动画数据到 {current_save_path}...")
        print(f"   平均速度: {avg_speed:.3f} m/s (vx={np.mean(root_vel_x_list):.3f}, vy={np.mean(root_vel_y_list):.3f})")
